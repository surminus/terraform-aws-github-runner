locals {
  ...
  logfiles = var.enable_cloudwatch_agent ? [for l in local.runner_log_files : {
    "log_group_name" : l.prefix_log_group ? "/github-self-hosted-runners/${local.resource_name}/${l.log_group_name}" : "/${l.log_group_name}"
...
resource "aws_ssm_parameter" "cloudwatch_agent_config_runner" {
  ...
  name  = "${local.resource_name}-cloudwatch_agent_config_runner"
  type  = "String"

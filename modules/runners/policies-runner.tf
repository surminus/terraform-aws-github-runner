...
resource "aws_iam_role" "runner" {
  name                 = "${local.resource_name}-runner-role"
...
resource "aws_iam_instance_profile" "runner" {
  name = "${local.resource_name}-runner-profile"
...
resource "aws_iam_role_policy" "ssm_parameters" {
  ...
  policy = templatefile("${path.module}/policies/instance-ssm-parameters-policy.json",
    {
      arn_ssm_parameters_prefix = "arn:${var.aws_partition}:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter/${local.resource_name}-*"
      arn_ssm_parameters_path   = "arn:${var.aws_partition}:ssm:${var.aws_region}:${data.aws_caller_identity.current.account_id}:parameter/${local.resource_name}/*"
    }
  )
}
